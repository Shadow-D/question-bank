...
class Login:
    ...
    def login():
        ...
        # 获取post数据中的随机字符串
        req = urllib.request.Request(hfut.Hfut.base_url, headers=hfut.Hfut.header)
        html = hfut.Hfut.opener.open(req).read().decode("utf-8")
        res = str(pattern1.search(html).group(0))
        code = res.split(" ")[2].split("\"")[1]

        # 识别验证码
        req = urllib.request.Request(url1 + time.strftime(gmt), headers=hfut.Hfut.header)
        img = hfut.Hfut.opener.open(req).read()
        ...
        # 登录后的页面源码
        req = urllib.request.Request(url2, post_data, headers=hfut.Hfut.header)
        html = hfut.Hfut.opener.open(req).read().decode('utf-8')
