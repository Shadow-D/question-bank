...
class SaveAnswer:
    ...
    def save_answer(save_url, save_data, question_json, config):
        ...
        req = urllib.request.Request(Hfut.base_url + save_url, save_data, headers=Hfut.header)
    ...
    def submit_answer(submit_url, is_submit):
        if is_submit == 'y':
            submit_header = Hfut.header
            submit_header['X-Requested-With'] = 'XMLHttpRequest'  # ajax请求标识
            req = urllib.request.Request(Hfut.base_url + submit_url, data=None, headers=submit_header)
            result = json.loads(Hfut.opener.open(req).read().decode('utf-8'))
    ...
    def submit_discuss_reply(reply_url, reply_data):
        ...
        req = urllib.request.Request(Hfut.base_url + reply_url, data=reply_data, headers=Hfut.header)
        result = Hfut.opener.open(req)
